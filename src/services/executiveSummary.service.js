...
function analyzeHealthTickets(tickets) {
  ...
  for (const ticket of tickets) {
    ...
    if (ticket.incidentType) {
      systemCount[ticket.incidentType] =
        (systemCount[ticket.incidentType] || 0) + 1;
    }
...
export async function generateExecutiveSummary(data, customerDisplayName) {
  try {
    ...
    if (data.detectionChartLabels && data.detectionChartData) {
      if (
        typeof data.detectionChartData === "object" &&
        data.detectionChartData.high
      ) {
    ...
    const totalHealthTickets = Array.isArray(data.healthTickets)
      ? data.healthTickets.length
      : 0;
    ...
    if (summary) {
      logger.info(
        `✅ Successfully generated executive summary for ${customerDisplayName}`
      );
      return summary;
    } else {
      logger.warn(
        `⚠️ Empty response from Azure OpenAI for ${customerDisplayName}`
      );
      return getDefaultExecutiveSummary(data, customerDisplayName);
    }
  } catch (error) {
    logger.error(
      `❌ Error generating executive summary for ${customerDisplayName}:`,
      error
    );
    return getDefaultExecutiveSummary(data, customerDisplayName);
  }
